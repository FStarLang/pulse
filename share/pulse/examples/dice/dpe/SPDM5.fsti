...
type spdm_measurement_result_t  = {
  ...
  sign_length : u32;
  sign: s:V.vec u8{V.length s == u32_v sign_length};
...
let parser_post1 (ctx:parser_context) (res:spdm_measurement_result_t)
                 ...
                 (is_sign_resp:bool) =
  match res.status with
  | Parse_error -> if is_sign_resp then 
                                    (
                                      exists* s_sign. V.pts_to res.sign s_sign **
                                                      pure (V.length res.sign == 0)
                                     //pure True 
                                    )
                                     else
                                    (
  ...
  | Signature_verification_error -> if is_sign_resp then 
                                    (
                                      exists* s_sign. V.pts_to res.sign s_sign **
                                                      pure (V.length res.sign == 0)
                                     //pure True 
  ...
  | Success ->                      if is_sign_resp then 
                                    (
                                      exists* resp_repr s_sign. valid_resp0 ctx resp_repr **
                                                          valid_measurement_blocks ctx 
                                                                                    res.measurement_block_vector 
                                                                                    resp_repr.measurement_record **
                                                           V.pts_to res.sign s_sign **
                                                           pure (V.length res.sign == u32_v res.sign_length /\
                                                                 V.length res.sign <> 0) 
                                    ...
                                    (
                                       exists* resp_repr. valid_resp0 ctx resp_repr **
                                                          valid_measurement_blocks ctx 
                                                                                    res.measurement_block_vector 
                                                                                    resp_repr.measurement_record **
                                                           pure (is_sign_resp ==> res.sign_length <> 0ul)
                                    )
