    ...
    ensures V.pts_to req #p_req b_req **
            ...
            (exists* tr1.
                  ...
                  pure ((fst res).status == Success ==>
                                      ...
                                      hash_result_success_no_sign tr0 tr1 #b_req #b_resp ))  
{
  let res = parser ctx #p_resp #b_resp false;
  match res.status {
    Parse_error -> {
      let tr1 = tr0;
    ...
    Success -> {
      ...
      match c {
        Initialized st -> {
          ...
          let ret = no_sign_resp_aux ctx req_size req c st rep res #tr0 #b_req #b_resp #p_req #p_resp;
              ...
              ))) 
{
    ...
    let tr1 = next_trace tr0 (G_Recv_sign_resp rep_new b_req b_resp);
    extend_trace ((get_state_data c).g_trace_ref) tr0 ((G_Recv_sign_resp rep_new b_req b_resp));
...
{
   let res = parser ctx #p_resp #b_resp true;
    match res.status {
      Parse_error -> {
        let tr1 = tr0;
        let r = (get_state_data c).g_trace_ref;
