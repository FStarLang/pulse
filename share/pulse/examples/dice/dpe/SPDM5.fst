...
let sign_resp_post_result_success (ctx:parser_context)
                                  ...
                                  (b_req : Seq.seq u8)
                                  (b_resp : Seq.seq u8)
                                  (res:spdm_measurement_result_t & state)
                                   : slprop =
              ...
              )
{
  ...
  match res.status {
    Parse_error -> {
      ...
      unfold (parser_post1 ctx res #b_resp true);
      
      assert_ (pure (res.status == Parse_error));
