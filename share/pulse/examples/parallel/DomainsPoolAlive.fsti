...
(*
Ideally, the following would be unobservable
--> Need to be able to read in unobservable
*)
```pulse
fn
...
(*
let rec close_task_bis_lemma
(t: task) (pos: nat)
(l: mono_list{task_in_queue t pos l /\ L.length l >= 1}):
Lemma (ensures get_actual_queue (close_task_bis t pos l) == get_actual_queue l /\ count_ongoing (close_task_bis t pos l) = count_ongoing l - 1)
= 
let (t', s)::q = l in
if pos + 1 = L.length l
  then (assume (s = Ongoing); ())
  //(t', Done)::q // if s is not Ongoing, then we have too much permission
  else close_task_bis_lemma t pos q
*)
...
ensures lock_thn (close_task_bis t pos l) q c
{
  ...
  with vq' vc'. assert (pts_to q vq' ** pts_to c vc' ** pure (link l vq' vc'));
  let vc: int = !c;
  c := vc - 1;
  close_task_bis_lemma t pos l;
...
(*
      ...
      pts_to status_pool true ** 
      lock_thn l q c ** 
      M.pts_to r one_half (close_task_bis t pos l)
*)
        //show_proof_state;
