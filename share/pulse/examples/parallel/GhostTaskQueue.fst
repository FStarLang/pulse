...
let witness #t #pos
  ...
  : stt_atomic (M.witnessed r (task_in_queue t pos)) #Unobservable emp_inames
                  (M.pts_to r v)
                  (fun _ -> M.pts_to r v)
= task_in_queue_stable t pos ; M.witness r (task_in_queue t pos) v
...
let recall #t #pos
 ...
 //stt_ghost unit 
  ...
  = M.recall r (task_in_queue t pos) w v
  ...
  decreases l
{
  ...
  if (pos + 1 = L.length l)
  {
    ...
    assert pure (et._2 == Ongoing);
    assert pure (get_actual_queue (close_task_bis t pos l) == get_actual_queue l);
    assert pure (count_ongoing (close_task_bis t pos l) = count_ongoing l - 1);
