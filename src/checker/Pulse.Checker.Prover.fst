...
let rec try_prove_core (g: env) (ctxt goals: list slprop_view) allow_amb : T.Tac (prover_result g ctxt goals) =
  ...
  | [] -> noop ()
  | goals ->
    debug_prover g (fun _ -> Printf.sprintf "proving %s from %s\n"
      (show (List.Tot.map elab_slprop goals))
      (show (List.Tot.map elab_slprop ctxt)));
...
let rec try_elim_core (g: env) (ctxt: list slprop_view) :
  ...
  let noop () : prover_result g ctxt [] =
    (| g, ctxt, [], [], fun g'' ->
      ...
      <: T.Tac _ |) in
  debug_prover g (fun _ -> Printf.sprintf "eliminating %s\n"
    (show (List.Tot.map elab_slprop ctxt)));
