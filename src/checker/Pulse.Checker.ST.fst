...
module ProverNew = Pulse.Checker.Prover
open Pulse.Show
...
let check
  ...
  | Some c -> (
    ...
    let (| g', ctxt', k |) = ProverNew.prove (RU.range_of_term e) g ctxt (comp_pre c) allow_ambiguous in
    let e = RU.deep_compress e in
    let ty = RU.deep_compress ty in
    ...
    let d : st_typing g' t c =
      ...
      RU.record_stats "prove_post_hint" fun _ -> ProverNew.prove_post_hint framed post_hint range
  )