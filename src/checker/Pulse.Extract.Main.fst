...
let rec simplify_st_term (g:env) (e:st_term) : T.Tac st_term =
  ...
  | Tm_Cleanup { cleanup_pre; handler; body } ->
    ret (Tm_Cleanup { cleanup_pre; handler = simplify_st_term g handler; body = simplify_st_term g body })
...
let rec erase_ghost_subterms (g:env) (p:st_term) : T.Tac st_term =
  ...
  then unit_tm
    ...
    | Tm_Cleanup { cleanup_pre; handler; body } ->
      ret (Tm_Cleanup { cleanup_pre; handler = erase_ghost_subterms g handler; body = erase_ghost_subterms g body })
