...
let mk_pure_let (b:binder) (head:term) (body:term) : term =
  let lb = U.mk_letbinding
    (Inl b.binder_bv) [] b.binder_bv.sort C.effect_PURE_lid head (lbattrs_of_binder b) FStarC_Range.dummyRange in
  S.mk (S.Tm_let {lbs=(false, [lb]); body1=body}) FStarC_Range.dummyRange
let mk_let (b:binder) (head:term) (body:term) : term =
  let lb = U.mk_letbinding
    (Inl b.binder_bv) [] b.binder_bv.sort C.effect_DIV_lid head (lbattrs_of_binder b) FStarC_Range.dummyRange in
  let tm_let =
