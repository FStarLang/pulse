...
let string_of_typestring (t: mlty): option string =
  let rec go t: option (list string) =
    ...
    | MLTY_Named ([c; t], p)
      when Syntax.string_of_mlpath p = "Pulse.C.Typestring.string_cons"
      ->
...
let int_of_typenat (t: mlty): option int =
  let rec go t =
    ...
    | MLTY_Named ([t], p)
      when Syntax.string_of_mlpath p = "Pulse.C.Typenat.s"
      ->
...
let my_types_without_decay () = 
  ...
  | MLTY_Named ([arg], p) when
      Syntax.string_of_mlpath p = "Pulse.C.Types.Base.ptr_gen"
    ->
      TBuf (translate_type_without_decay env arg)

  | MLTY_Named ([arg], p) when
    Syntax.string_of_mlpath p = "Pulse.C.Types.Scalar.scalar_t"
    ->
  ...
  | MLTY_Named ([t; n; s], p)
    when false
     || Syntax.string_of_mlpath p = "Pulse.C.Types.Array.base_array_t"
    ->
      TArray (
...
let my_exprs () = register_pre_translate_expr begin fun env e ->
  ...
  | MLE_App ({ expr = MLE_TApp({ expr = MLE_Name p }, _) }, [ _ (* typedef *) ])
    when (
          string_of_mlpath p = "Pulse.C.Types.Base.alloc" ||
          false) ->
      EBufCreateNoInit (ManuallyManaged, EConstant (UInt32, "1"))

  | MLE_App ({ expr = MLE_TApp({ expr = MLE_Name p }, _) }, [ _ (* td *); sz ])
    when (
          string_of_mlpath p = "Pulse.C.Types.Array.array_ptr_alloc" ||
          false) ->
      EBufCreateNoInit (ManuallyManaged, translate_expr env sz)

  | MLE_App ({ expr = MLE_TApp({ expr = MLE_Name p }, _) }, [ _(* td *); _ (* s *); e2; _ (* len *) ])
    when (
          string_of_mlpath p = "Pulse.C.Types.Array.array_ref_free" ||
          false) ->
      EBufFree (translate_expr env e2)

  | MLE_App ({ expr = MLE_TApp({ expr = MLE_Name p }, _) }, [ _ (* typedef *); _ (* v *); e ]) when
       string_of_mlpath p = "Pulse.C.Types.Base.free" ->
      EBufFree (translate_expr env e)

(* BEGIN support for the Pulse.C null pointer. *)

  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, [t])}, [ _ (* td *); _ (* v *); e; _ (* len *) ])
    when string_of_mlpath p = "Pulse.C.Types.Array.array_ptr_is_null"
    -> generate_is_null (translate_type env t) (translate_expr env e)

  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, [t])}, [ _ (* td *); _ (* v *); e])
    when string_of_mlpath p = "Pulse.C.Types.Base.is_null"
    -> generate_is_null (translate_type env t) (translate_expr env e)
  
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, [t])}, _)
    when false
    || Syntax.string_of_mlpath p = "Pulse.C.Types.Base.null_gen"
    || Syntax.string_of_mlpath p = "Pulse.C.Types.Array.null_array_ptr"
    -> EBufNull (translate_type env t)

  | MLE_TApp ({expr=MLE_Name p}, [t]) when
      string_of_mlpath p = "Pulse.C.Types.Base.null_gen"
    -> EBufNull (translate_type env t)

(* END support for the Pulse.C null pointer *)
  ...
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, _)},
    ...
    when string_of_mlpath p = "Pulse.C.Types.Struct.struct_field0"
    || string_of_mlpath p = "Pulse.C.Types.Union.union_field0"
  ...
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, (t :: _))},
    ...
    when string_of_mlpath p = "Pulse.C.Types.UserStruct.struct_field0"
    ->
      EAddrOf (EField (
  ...
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, _)}, [
    ...
    when string_of_mlpath p = "Pulse.C.Types.Base.void_ptr_of_ptr" ->
      ECast(
        translate_expr env x,
        TAny
      )
  ...
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, [t])}, [
    ...
    when string_of_mlpath p = "Pulse.C.Types.Base.ptr_of_void_ptr" ->
      ECast(
        translate_expr env x,
  ...
  | MLE_App ({expr=MLE_TApp ({expr=MLE_Name p}, _)}, [
    ...
    when string_of_mlpath p = "Pulse.C.Types.Array.array_ref_cell"
    || string_of_mlpath p = "Pulse.C.Types.Array.array_ref_split"
    ->
  ...
  | MLE_App ({ expr = MLE_TApp({ expr = MLE_Name p }, _) }, [
      ...
      string_of_mlpath p = "Pulse.C.Types.Array.array_blit_ptr"
    ) ->
...
let parse_steel_c_fields env (fields: mlty): option (list _) =
      let rec go fields =
        ...
        | MLTY_Named ([], p)
          when false
          || Syntax.string_of_mlpath p = "Pulse.C.Types.Fields.field_t_nil"
          -> Some []
          
        | MLTY_Named ([field; t; fields], p)
          when false
          || Syntax.string_of_mlpath p = "Pulse.C.Types.Fields.field_t_cons"
          ->
...
let my_type_decls () = register_pre_translate_type_decl begin fun env ty ->
    ...
    | {tydecl_defn=Some (MLTD_Abbrev (MLTY_Named ([tag; fields; _; _], p)))}
      when Syntax.string_of_mlpath p = "Pulse.C.Types.Union.define_union0"
      ->
