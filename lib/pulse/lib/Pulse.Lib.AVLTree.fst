...
```pulse
fn is_empty (#t:Type) (x:tree_t t) (#ft:(T.tree t))
  requires is_tree x ft
  returns b:bool
  ensures is_tree x ft ** pure (b <==> (ft == T.Leaf))
{
  match x {
    None -> {
      is_tree_case_none x;
...
```pulse
fn rec is_mem (#t:eqtype) (x:tree_t t) (v: t) (#ft:G.erased (T.tree t))
    requires is_tree x ft
    returns b:bool
